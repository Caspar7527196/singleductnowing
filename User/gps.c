...
sGPS gps[GPS_NUM];
/******************驱动程序****************/
void USART6_IRQHandler(void)        //空闲中断+DMA        	
{ 
//	Led_Tog(2);
	sGPS *ele=gps;
...
bool USART6_Transmit_DMA(u8 *pData, u16 Size)  //DMA发送
{
	sGPS *ele=gps;
...
//	DMA_CLEAR_FLAG_ALL(ele->huart->hdmarx);
	...
	ele->huart->hdmatx->Instance->NDTR = GPS_TX_LEN;
...
//	static u16 Cali=0;  //下次进来就不用等待了
	if((ele->star<=12||ele->gpsPosAccuracy > 1.5f)&& !ele->ECEF_Init_Flag)
	{
//		if((Cali<100))
//		{
//			Cali++;
...
//	if(ms5611[0].AltOff==0)  return false;  //气压计未完成初始化
	/****计算磁偏角****/
	ele->yaw_declination = get_declination(ele->lat,ele->lng);
	/****计算位置*****/
	ele->LLH_Init[0] = ele->lat*D2R;         //纬度
	ele->LLH_Init[1] = ele->lng*D2R;         //经度
	ele->LLH_Init[2] = ele->alti;  //ms5611->AltRel;//高度，注意是该减还是加
	//LLH换算成ECFF（地球中心坐标系）
...
bool Gps_Calc(sGPS *ele)
{
	...
	do{
		...
		for(u8 i=0;i<num;i++)
		{
			...
			if(*(RxType[i]+4)=='G')
			{
				...
				ele->status=atoi(RxPara[5]);           //第七位GPS状态
			...
			if(*(RxType[i]+4)=='S')
			{
				...
				if(*(RxPara[0])=='1'){
					...
					Tim_Calc(&tim_tmp_mag);
					ele->mag_update_time = tim_tmp_mag.CNT;
					ele->MagUpdate = true;
				}
				else
				{
				...
				if(*(RxPara[0]+1)=='1'){
					...
					gps->gpsPosAccuracy = atof(RxPara[11]);
					gps->gpsHeiAccuracy = atof(RxPara[12]);//..高度精度
					gps->gpsSpdAccuracy = atof(RxPara[13]);
					gps->gps_headAccuracy = atof(RxPara[14]);
					gps->gps_heading =  atof(RxPara[15]);
					gps->status = atoi(RxPara[16]);
					gps->GPSUpdate = 1;
					Tim_Calc(&tim_tmp_gps);
					gps->gps_update_time = tim_tmp_gps.CNT;
				}
			}
		}
	}while(0);
	...
	if(GpsFlag == true)
	{
		...
		//初始化ECEF坐标系
		Gps_Cali(ele);	
//		if(ele->ECEF_Init_Flag)
//		{
//			ele->LLH[0] = ele->lat * D2R;
//			ele->LLH[1] = ele->lng * D2R;
//			if(gps->alti_off == 0 && gps->gpsHeiAccuracy < 1.0f)//..在非RTK模式下要把0.3调大才可以用高度卡尔曼
//			{
//				gps->alti_off = ele->alti;
//			}
//			ele->LLH[2] = ele->alti - gps->alti_off;
